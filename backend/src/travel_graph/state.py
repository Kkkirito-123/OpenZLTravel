...
``draft``、``budget`` 和路线修改指令是图内结果；警告和 ``trip_id`` 用于确认与最终
展示。节点异常由 LangGraph Run 状态表示，不伪造图内失败分支。用户身份放在
``TravelContext``，初始令牌放在 ``TravelInput``，二者
都不作为普通旅行事实写入业务状态。
"""
...
TravelPhase = Literal["planning", "awaiting_route_confirmation", "completed"]
