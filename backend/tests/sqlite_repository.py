...
class TripRepository(Protocol):
    ...
    def save(
        ...
        visitor_id: UUID = TEST_VISITOR_ID,
    ) -> None:
        """保存请求与完整行程快照。"""
    ...
    def get(
        self, trip_id: UUID, visitor_id: UUID = TEST_VISITOR_ID
    ) -> Itinerary | None:
        """按 ID 读取完整行程。"""
...
class SqliteTripRepository:
    ...
    def list(self, visitor_id: UUID = TEST_VISITOR_ID) -> list[TripSummary]:
        """按创建时间倒序读取历史行程摘要。"""
    ...
    def delete(self, trip_id: UUID, visitor_id: UUID = TEST_VISITOR_ID) -> bool:
        """删除指定行程，并返回是否实际删除。"""
    ...
    def save_session(
        self, session: PlanningSession, visitor_id: UUID = TEST_VISITOR_ID
    ) -> None:
    ...
    def get_session(
        self, session_id: UUID, visitor_id: UUID = TEST_VISITOR_ID
    ...
    def list_recoverable_sessions(
        ...
        return [
            (TEST_VISITOR_ID, PlanningSession.model_validate_json(row["session_json"]))
            for row in rows
        ]
    ...
    def create_dialogue(
        self, state: TravelDialogueState, visitor_id: UUID = TEST_VISITOR_ID
    ) -> None:
    ...
    def list_memories(
        self, visitor_id: UUID = TEST_VISITOR_ID
    ) -> builtins.list[TravelMemory]:
    ...
    def delete_memory(
        self, key: MemorySlotName, visitor_id: UUID = TEST_VISITOR_ID
    ) -> bool:
