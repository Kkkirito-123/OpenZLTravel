...
describe("TripMap", () => {
  it("只使用后端返回的真实路线轨迹", async () => {
    ...
    const path = (polyline.mock.calls[0][0] as { path: [number, number][] }).path;
    expect(path).toHaveLength(3);
    expect(path[0][0]).toBeCloseTo(120.10479, 4);
    expect(path[0][1]).toBeCloseTo(30.09771, 4);
    ...
    routes: [
      {
        ...
        source: { provider: "amap", freshness: "realtime" },
      },
    ],
      weather: {
      date: "2026-09-01",
