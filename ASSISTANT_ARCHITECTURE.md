...
意图模型不设置单次输入、单次输出或会话累计 Token 硬限制：

- 快速解析成功时不调用模型。
- 单次意图调用和一次结构修复只受 15 秒超时约束，不向供应商发送 `max_tokens`。
- 供应商返回 `usage` 时记录权威值；未返回时使用 OpenZLAgent 的保守估算。

只累计成功进入状态事务的意图调用，精确结果缓存命中记为零模型调用。滚动摘要由独立的
3,000 Token 近期窗口、
