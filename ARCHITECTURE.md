...
| 层 | 职责 |
|---|---|
| `backend/src/assistant/` | 无数据库会话恢复、LangChain Agent、只读工具、SSE 事件、工单签发 |
| `backend/src/travel_graph/` | 工单认证、确定性日程、最终路线、预算、路线确认、幂等保存 |
| `backend/src/domain/` | TravelOrder、事实模型、确定性规划和校验 |
| `backend/src/providers/` | Catalog、12306、RollingGo、天气和路线事实 Provider |
| `backend/src/runtime/` | Settings、Protocol、依赖装配、HMAC Token |
| `backend/src/api/` | 匿名身份、历史行程、Agent Server 授权 |
| `frontend/src/` | 唯一 AI 页面、Assistant SSE、Graph Thread/Run、卡片和行程展示 |
